...
- **⛔ FALSE FACT, KILLED TWICE, DO NOT REVIVE: "clangd holds `obj/gen` headers open."** It does **not**. clangd reads file contents into its **in-memory VFS** and does not retain long-lived OS handles on parsed headers; any mmap during a read is transient and racy, not "holds open." This claim was flagged false by Task 8's review, then **re-asserted in a new location by Task 9** and flagged false again — it propagates because this repo's `CLAUDE.md` makes comments the durable rationale store, so the next reader trusts it and reuses it to justify a *different* decision where it may not be harmless. **If you need to justify tolerating a locked generated file, use the true and broader ground:** *any* process may transiently hold one — an indexer, antivirus, a concurrent build, an open editor, clangd — so a hard failure there lets an unrelated process break the build. That's true regardless of clangd's internals. **Tasks 11-13: you are designing against real clangd. Verify its behavior; do not inherit this.**
...
>
> **RESOLVED in Task 11's review round: `ExecutableLocator.Find` now normalizes via `Path.GetFullPath`, so it IS absolute.** Both the implementer and I first said "document, don't normalize" (fearing an unasked-for CWD-resolution change); the code reviewer overturned us with the better argument: **`File.Exists` had already resolved the relative path against the CWD at that same instant**, so `GetFullPath` on the hit pins exactly the file just proven to exist — making an existing *implicit* resolution explicit and durable, not changing behavior. Identical when the CWD is unchanged, correct when it isn't. It lives in `Find` (the impure wrapper that already owns the environment), **not** `FindIn` — normalizing in the pure core would reintroduce an environment read and make test expectations CWD-dependent.
>
> Verified behaviors from Task 11, so nobody re-derives them:
> - **The returned extension is spelled from `PATHEXT`** (`clangd.EXE`), **not from disk** (`clangd.exe`). Functional — Windows paths are case-insensitive, which is how the match was made — but it is **not the file's canonical name. Don't display it as one.** (This one is correctly *documented and pinned* rather than normalized: getting the on-disk spelling needs directory enumeration per candidate, and the path opens the same file either way.)
> - Search is **directory-major**: PATH order outranks PATHEXT order, matching Windows.
> - **The bare name does NOT lead on Windows.** Task 11's first draft put it first, justified as "preserving the predecessor's behavior for extensionless names" — but the review found **no caller ever passed a bare name**, so there was no such behavior to preserve. Left in, it meant an extensionless `clangd` shim (a normal MSYS2 / Cygwin / git-for-windows shape) would beat the real `clangd.exe` **in the same directory** — returning a file Windows would never execute, for Task 12 to spawn. Fixed in the review round.
...
- **Test-side clangd discovery is duplicated twice** — `LocateClangd`/`ProbeVgsToolsDir`/`RequireClangd` live in both `ClangdLaunchTests` and `ClangdE2ETests`. Two copies are acceptable today; when 3b's clangd download adds a THIRD consumer of the `~/.vgs/tools` discovery logic, extract the trio into a shared `ClangdTestDiscovery` helper instead of copying again.
