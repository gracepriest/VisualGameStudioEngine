...
- **D9 — Entry-point scanning of user C++ is a comment/string-stripped textual heuristic.** `int|auto main(`, `wmain(`, `WinMain(`, `wWinMain(` count as candidate DEFINITIONS (followed by `{`, allowing trailing `-> int`, `noexcept`/`noexcept(...)`, and function-try `try`). The stripper handles `//`/`/* */` comments (incl. backslash line-splice), string/char literals with escapes, C++14 digit separators (`10'000`), and raw string literals (`R"(...)"`). Preprocessor-conditional mains can overcount, and a UTF-8 char literal of a hex letter (`u8'a'`) can be misread as a digit separator — both documented limitations; the error names every candidate file so the user can restructure. Pre-link enforcement is required because 0/2-main linker errors don't parse (recon). (Scanner hardened in Task 3 after adversarial review found valid-C++ false-error triggers.)
