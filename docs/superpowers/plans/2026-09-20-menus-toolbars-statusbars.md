...
- [ ] **Web:** NOT RUN on this machine — `basiclang build` of a web project dies with `ERROR_USER_MAPPED_FILE` on a fresh `App.js`, the same deterministic failure every web acceptance test on this branch hits. Master's write-once/rename JS-emitter fix is expected to clear it; re-run this bullet after the merge to master (see `docs/HANDOFF.md`'s Task 24e subsection).
...
- [ ] Memory: `form-designer-sep18.md` + `MEMORY.md` — explicitly OUT OF SCOPE for this docs-only pass (memory files are outside the repo and were not touched); left for the session that runs Task 30's gate.
- [ ] Tick every box in this plan. — done for Tasks 26, 27, 29 and the WinForms half of 28; the Web bullet of 28 and Task 30 are left open on purpose (not yet run/gated).
