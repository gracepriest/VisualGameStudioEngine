...
> - **Decimal→NON-FLOATING conversions are unguarded on BOTH lowering routes** — sweep the whole family, not just integers: `CType(d, Integer)`/`CType(d, String)`/`CType(d, Boolean)` (Visit(IRCast)) and `CInt(d)`/`CLng(d)`/`CStr(d)`/`CBool(d)` (the EmitStdLibCall intrinsic route) would emit `static_cast<int32_t>` / `std::to_string` / `static_cast<bool>` on the struct — all invalid C++. Verify what the analyzer permits, then per target: integral → `(d).ToDouble()` + cast with DOCUMENTED truncation semantics (real .NET truncates toward zero for `(int)decimal`); String → `(d).ToString()`; Boolean → adjudicate (reject cleanly is acceptable). Anything not lowered must fail in the member/capability pass with a clean diagnostic, never as raw C++.
...
- [ ] **Step 4b: Add these implementation-discovered rows to spec §13's documented-divergence list** (recorded during Task 10; both are BEHAVIOUR the tests already pin, not spec changes):
  - **0-argument `New` is rejected for EVERY P1 value type**, not just Guid: the v1 surface ctor arities are DateTime `(3,6)`, TimeSpan `(3,4)`, DateTimeOffset `(1,2)`, Guid `(1)`, and Decimal has NO ctor row at all — so `New DateTime()` / `New Guid()` / `New Decimal()` all get the clean arity diagnostic where .NET has a parameterless ctor returning the zero value. The BL equivalents are the zero-init default (`Dim d As DateTime`, which the `{}` fallback gives exactly `DateTime.MinValue`/`TimeSpan.Zero`/`Guid.Empty`/`0D`) or the `X.Empty`/`X.Zero`/`X.MinValue` statics. Adding parameterless ctor rows later is purely additive.
  - **Decimal → integral narrows through `ToDouble()`**, so it inherits the .NET VarR8FromDec 15-significant-digit rule: a Decimal carrying more than 15 significant digits converts APPROXIMATELY (the C# backend's `(int)d` is exact), and a value outside the target integral's range is UNDEFINED BEHAVIOUR in C++ where .NET throws OverflowException. **Task 13 constraint:** parity programs doing Decimal→integral must keep operands ≤15 significant digits AND in range. (Truncation direction itself is faithful — both `(int)decimal` and the C++ floating→integral cast truncate toward zero.)
