...
### Plan 2d — DONE Sep 10 (a JavaScript project TYPE in the IDE and the CLI)

- **Before this, nothing in the IDE could CREATE a JavaScript project.** The wizard offered four BasicLang backends, `ProjectTemplateService` mapped solution types to `<TargetBackend>` with a `_ => "CSharp"` default, and no template's `SupportedSolutionTypes` named JavaScript. The build (`BuildService`), F5 (`StartJavaScriptPreviewAsync` + `WebPreviewServer`) and the `.blproj` loader already understood `TargetBackend.JavaScript` — a hand-written project worked, a wizard-made one could not exist. Same shape as the three dispatch maps in plan 1: a missing arm did not fail, it silently produced C#.
- **What landed:** `SolutionTypes.JavaScript` (id `javascript`, "JavaScript (Web)", `.bas` source) · `ProjectTemplates.WebSite` (id `web-site`, Category Web, `javascript`-only because the page reaches the DOM through `::document`, which no other backend lowers) · the `"javascript" => "JavaScript"` arm, **and the switch default now THROWS** — `ProjectTemplateBackendMappingTests` pins every id in `SolutionTypes.All` against its backend, that every id has a template, and that an unmapped id is a creation error rather than a C# project · a wizard `BackendOption` "JavaScript (Web)" between Native C++ and LLVM, not probe-gated (no toolchain) · CLI `basiclang new web`, its `Main.bas` held byte-identical to the IDE template by `TemplateBuildSweepTests` (the same contract the C++ pairs carry).
- **The template page is the shape verified in a browser when Task 28 closed:** a typed DOM entry (`Dim doc As Document = ::document`), a class with an `Event`, `AddHandler` with a single-line `Sub` lambda, `addEventListener` with a lambda, `Console.WriteLine`. The sweep builds it with the real compiler on BOTH routes and asserts the SITE — index.html, the script and its map under `bin\`, a DOM call surviving codegen — because a JavaScript build that wrote nothing would still exit 0. Re-verified by hand: `basiclang new web` → `build` → served → heading renders, three clicks read "Clicked 3 times", console shows the load message.
- **Three defects the review of this work found, each REPRODUCED before it was fixed** (the review's refuters died on a usage limit, so nothing arrived verified — measuring was the whole job):
  - ⛔⛔ **A URL-significant character in the project name was a SILENT BLANK PAGE.** `new web -n "Site #1"` built exit 0 and wrote an index.html carrying the RAW file name; MEASURED in a browser: the src split at the `#`, the request went to `/Site%20`, 404, nothing rendered, build still green. `#` and `%` are legal file-name characters the wizard admits. `JavaScriptEmitter` now percent-encodes the harness `src` **and** the `sourceMappingURL` (also a URL — unencoded, it silently costs the source view). ⭐ The existing test asserted `src="a&amp;b.js"`: well-formed HTML naming a file that happens to resolve, so it **could not tell a correct URL from a broken one** — HTML-escaping and URL-encoding are different escapings and the test conflated them.
  - **`basiclang new web` told the user to run `basiclang run`, which always failed** — `HandleRunCommand` probes only for a `.dll`/`.exe` and exited 1 with "Output not found". The scaffold's own instructions ended in an error. `run` now names the `index.html` and returns 0; the web template's next steps say the same. Nothing is launched: ES modules do not load over `file://` and the HTTP server lives in the IDE, not the compiler.
  - **A name containing `&` produced a .blproj that is not well-formed XML** — creation succeeded, the first build died on the XML parser. Pre-existing for EVERY template in BOTH systems. Escaped now where it lands in XML, raw everywhere else. ⚠ `.blsln` is **excluded on purpose**: the CLI's `sln` template emits the legacy PLAIN-TEXT format, so **extension alone cannot decide what needs escaping** (found while writing the fix, pinned by its own test).
  - Also pinned: the .blproj the wizard writes must **round-trip through the IDE's own `ProjectSerializer`**. Asserting the text alone would still pass if the loader could not parse the token — the difference between a project that builds as JavaScript and one that reopens as C#.
- **Not done:** a second template (a page importing a library via `#JsImport`); a per-backend hint on the configure page (the select page already shows the solution type's description, and no AXAML change was needed).
