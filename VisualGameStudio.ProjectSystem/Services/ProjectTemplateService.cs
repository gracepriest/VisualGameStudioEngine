...
public class ProjectTemplateService : IProjectTemplateService
{
    ...
    private string GenerateProjectFileContent(CreateProjectOptions options)
    {
        ...
        sb.AppendLine($"# BasicLang Project File");
        sb.AppendLine($"# Generated by Visual Game Studio");
        sb.AppendLine();
        sb.AppendLine($"[project]");
        sb.AppendLine($"name = \"{options.Name}\"");
        sb.AppendLine($"type = \"{options.Template.Id}\"");
        sb.AppendLine($"target = \"{options.SolutionType.Id}\"");
        sb.AppendLine($"namespace = \"{options.Namespace ?? options.Name}\"");
        sb.AppendLine($"version = \"1.0.0\"");
        sb.AppendLine();
        sb.AppendLine($"[build]");
        sb.AppendLine($"output_type = \"{GetOutputType(options.Template)}\"");

        if (options.SolutionType.Id == "dotnet" || options.SolutionType.Id == "msil")
        {
            sb.AppendLine($"target_framework = \"{options.TargetFramework}\"");
        }

        sb.AppendLine();
        sb.AppendLine($"[sources]");
        sb.AppendLine($"include = [\"**/*{options.SolutionType.SourceExtension}\"]");
        sb.AppendLine();

        if (options.Template.Dependencies.Any())
        {
            sb.AppendLine($"[dependencies]");
            foreach (var dep in options.Template.Dependencies)
            {
