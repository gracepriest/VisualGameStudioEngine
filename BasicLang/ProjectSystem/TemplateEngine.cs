...
"
            ...
            // Web site — BasicLang compiled to JavaScript (`basiclang new web`).
            // The build writes index.html + the script + its source map under
            // bin\ and stops there; no toolchain is involved.
            // NOTE: Main.bas must stay in sync with the IDE's web-site template
            // in VisualGameStudio.ProjectSystem/Services/ProjectTemplateService.cs
            // (GenerateWebSiteMain) — TemplateBuildSweepTests enforces it.
            _templates["web"] = new ProjectTemplate
            {
                Name = "web",
                DisplayName = "Web Site",
                Description = "A browser web site compiled to JavaScript",
                ShortName = "web",
                DefaultProjectName = "WebSite",
                Tags = new List<string> { "web", "javascript", "js", "browser" },
                Files = new Dictionary<string, string>
                {
                    ["{{ProjectName}}.blproj"] = @"<BasicLangProject Version=""1.0"">
  <PropertyGroup>
...
",
            ...
            //
            // ⛔ The SOURCE FILES here are the IDE's winforms-app template verbatim
            // (ProjectTemplateService.GenerateWinFormsMain / GenerateWinFormsForm, with {ns}
            // rewritten to the CLI's {{ProjectName}} placeholder), which is itself the canonical
            // VSIX shape (Owner decision 3). Three implementations of one template existed and had
            // already drifted into three different shapes; WinFormsTemplateBuildTests compares
            // them, so change one and change all.
            //
            // ⚠ Intentional .blproj divergence, matching the cpp templates' note above: the CLI
            // emits defaults only. Only the SOURCE files are held identical.
            _templates["winforms"] = new ProjectTemplate
            {
                Name = "winforms",
                DisplayName = "Windows Forms Application",
                Description = "A Windows desktop application using Windows Forms",
                ShortName = "winforms",
                DefaultProjectName = "MyWinFormsApp",
                Tags = new List<string> { "winforms", "desktop", "windows", "gui" },
                Files = new Dictionary<string, string>
                {
                    ["{{ProjectName}}.blproj"] = @"<BasicLangProject Version=""1.0"">
  <PropertyGroup>
    ...
    <!-- Without a DPI mode WinForms runs legacy-unaware, and the designer's pixel
         coordinates and the running window's disagree on any scaled display. -->
  ...
  <ItemGroup>
    ...
    <Compile Include=""MainForm.bas"" />
                    ...
                    ["Main.bas"] = @"' {{ProjectName}} - Windows Forms Application
' Generated by Visual Game Studio
...
Module Program
    ...
    End Sub
End Module",
                    ["MainForm.bas"] = @"' MainForm.bas - Main application window
' {{ProjectName}}
...
Public Class MainForm
    ...
    Private Sub InitializeComponent()
        ...
        lblMessage.Text = ""Hello, {{ProjectName}}!""
    ...
    Private Sub btnClick_Click(sender As Object, e As EventArgs)
        MessageBox.Show(""Button clicked!"", ""{{ProjectName}}"", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub
End Class",
                    [".gitignore"] = "bin/\nobj/\n"
                }
