using NUnit.Framework;
using BasicLang.Compiler;
using BasicLang.Compiler.AST;
using BasicLang.Compiler.SemanticAnalysis;
using BasicLang.Compiler.IR;
using BasicLang.Compiler.CodeGen;
using BasicLang.Compiler.CodeGen.CSharp;

namespace VisualGameStudio.Tests.Compiler;

/// <summary>
/// Explicit generic type arguments on a method call — Method(Of T)() — must
/// survive to the generated C# as Method&lt;T&gt;(). Dropping them (as the
/// Avalonia template's AppBuilder.Configure(Of App)() hit) changes semantics.
/// </summary>
[TestFixture]
public class GenericMethodCallTests
{
    private string CompileToCSharp(string source, out List<string> errors)
    {
        errors = new List<string>();

        var lexer = new Lexer(source);
        var tokens = lexer.Tokenize();

        var parser = new Parser(tokens);
        ProgramNode ast;
        try
        {
            ast = parser.Parse();
        }
        catch (Exception ex)
        {
            errors.Add($"Parse error: {ex.Message}");
            return null;
        }

        var analyzer = new SemanticAnalyzer();
        if (!analyzer.Analyze(ast))
        {
            foreach (var err in analyzer.Errors)
                errors.Add($"Semantic error: {err.Message}");
            return null;
        }

        var irBuilder = new IRBuilder(analyzer);
        var irModule = irBuilder.Build(ast, "TestModule");

        var csharpGen = new ImprovedCSharpCodeGenerator(new CodeGenOptions
        {
            Namespace = "TestOutput",
            GenerateMainMethod = false,
            GenerateComments = false
        });
        return csharpGen.Generate(irModule);
    }

    [Test]
    public void StaticGenericMethodCall_KeepsTypeArgument()
    {
        var source = @"Module M
    Public Class App
    End Class

    Sub Main()
        AppBuilder.Configure(Of App)()
    End Sub
End Module";

        var output = CompileToCSharp(source, out var errors);
        Assert.That(output, Is.Not.Null, "compile failed: " + string.Join("; ", errors));
        Assert.That(output, Does.Contain("Configure<App>"),
            "explicit generic type argument must survive to the C# call");
    }
}
